...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    /**
     * Handles the exception thrown when a Pokémon or Ability is not found during a search.
     * Logs the exception information using log4j and returns an error response in the form of {@link ErrorResponseDTO}.
     *
     * @param ex      The exception instance of either {@link PokemonNotFoundException} or {@link AbilityNotFoundException} or {@link MoveNotFoundException}
     * @param request The web request
     * @return A {@link ResponseEntity} object with the error response and the appropriate HTTP status code.
     */

    @ExceptionHandler(value = {PokemonNotFoundException.class, AbilityNotFoundException.class, MoveNotFoundException.class})
    protected ResponseEntity<Object> handleNotFoundException(final RuntimeException ex, WebRequest request) {
        ...
        final ErrorResponseDTO errorResponse =
                ErrorResponseDTO.builder()
                        ...
                        .message("Oops! It looks like we couldn't find any results with the search parameters you specified.")
    ...
    public ResponseEntity<Object> handleAnyException(final Exception ex, final WebRequest request) {
        ...
        final ErrorResponseDTO errorResponse =
                ErrorResponseDTO.builder()
                        ...
                        .message("Oops! It seems there's an issue. The server experienced an unexpected error.")
                        .errors(Collections.singletonList(ex.getLocalizedMessage()))
                        .path(request.getDescription(false).substring(4))
                        .build();
        return new ResponseEntity<>(errorResponse, new HttpHeaders(), errorResponse.getStatus());
